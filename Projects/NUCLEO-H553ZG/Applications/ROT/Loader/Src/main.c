...
/*int main(void) */
{
  ...
  */
  ...
  /* USER CODE BEGIN 2 */
#ifdef USB_CDC_SUPPORT
  printf("Welcome to USB CDC ACM Test Demo!\r\n");
	printf("Plug USB cable to type-C port and open another COM terminal to receive test data...\r\n");
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  HAL_Delay(1000);
		...
		{
			sprintf(gSendBuf,"Test Message:%d\r\n", num++);
			CDC_Transmit_FS((uint8_t *)&gSendBuf[0], strlen(gSendBuf));
		}
  }
#else
  NVIC_SystemReset();
...
{
	//send back the same data to host
