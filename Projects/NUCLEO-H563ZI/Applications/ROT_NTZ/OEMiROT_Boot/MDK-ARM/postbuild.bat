...
set "loader_dir=%projectdir%\..\..\OEMiROT_Loader"
...
set "command=%python%%applicfg% flash --layout %preprocess_bl2_file% -b ext_loader -m RE_LOADER --decimal %img_config% --vb >> %current_log_file% 2>&1"
%command%
IF !errorlevel! NEQ 0 goto :error
...
set loader_main="%loader_dir%\Inc\main.h"
...
set "command=%python%%applicfg% flash --layout %preprocess_bl2_file% -b MCUBOOT_PRIMARY_ONLY -m RE_PRIMARY_ONLY %map_properties% --vb >> %current_log_file% 2>&1"
%command%
IF !errorlevel! NEQ 0 goto :error

set "command=%python%%applicfg% flash --layout %preprocess_bl2_file% -b APP_DATA_REGION_START -m RE_IMAGE_FLASH_ADDRESS_DATA %map_properties% --vb >> %current_log_file% 2>&1"
%command%
IF !errorlevel! NEQ 0 goto :error
...
set "command=%python%%applicfg% flash --layout %preprocess_bl2_file% -b LOADER_REGION_START -m RE_LOADER_CODE_START %map_properties% --vb >> %current_log_file% 2>&1"
%command%
IF !errorlevel! NEQ 0 goto :error

set "command=%python%%applicfg% flash --layout %preprocess_bl2_file% -b LOADER_REGION_SIZE -m RE_LOADER_CODE_SIZE %map_properties% --vb >> %current_log_file% 2>&1"
%command%
IF !errorlevel! NEQ 0 goto :error
...
:: =============================================================== Update %loader_sct_file% ================================================================
set "command=%python%%applicfg% linker --layout %preprocess_bl2_file% -m RE_LOADER_CODE_START -n LOADER_CODE_START %loader_sct_file% --vb >> %current_log_file% 2>&1"
%command%
IF !errorlevel! NEQ 0 goto :error

set "command=%python%%applicfg% linker --layout %preprocess_bl2_file% -m RE_LOADER_CODE_SIZE -n LOADER_CODE_SIZE %loader_sct_file% --vb >> %current_log_file% 2>&1"
%command%
IF !errorlevel! NEQ 0 goto :error

:: =============================================================== Update %loader_main% ================================================================
set "command=%python%%applicfg% definevalue --layout %preprocess_bl2_file% -m RE_LOADER_CODE_START -n LOADER_CODE_START %loader_main% --vb >> %current_log_file% 2>&1"
echo %command%
%command%
IF !errorlevel! NEQ 0 goto :error
...
set "command=%python%%applicfg% definevalue --layout %preprocess_bl2_file% -m RE_IMAGE_FLASH_APP_IMAGE_SIZE -n FLASH_PARTITION_SIZE %appli_flash_layout% --vb >> %current_log_file% 2>&1"
%command%
IF !errorlevel! NEQ 0 goto :error

set "command=%python%%applicfg% definevalue --layout %preprocess_bl2_file% -m RE_IMAGE_FLASH_DATA_IMAGE_SIZE -n FLASH_DATA_PARTITION_SIZE %appli_flash_layout% --vb >> %current_log_file% 2>&1"
%command%
IF !errorlevel! NEQ 0 goto :error

set "command=%python%%applicfg% definevalue --layout %preprocess_bl2_file% -m RE_LOADER_CODE_START -n LOADER_CODE_START %appli_flash_layout% --vb >> %current_log_file% 2>&1"
%command%
IF !errorlevel! NEQ 0 goto :error
