...
static void SetProductState(uint32_t prodState)
{
  ...
  printf("After OB_Launch: shouldn't be here !\r\n");
  Error_Handler();
...
int main(void)
{
 ...
 /* Enable SecureFault handler (HardFault is default) */
  ...
  HAL_GPIO_ConfigPinAttributes(COM1_RX_GPIO_PORT, COM1_RX_PIN, GPIO_PIN_NSEC);
  HAL_GPIO_ConfigPinAttributes(COM1_TX_GPIO_PORT, COM1_TX_PIN, GPIO_PIN_NSEC);
  ...
  /* Stop the systick during test menu execution */
#if 1
  HAL_SuspendTick();
  {
    ...
    while(loop == 1)
    {
      ...
      printf("\r\n ======== Test menu ======== \r\n");
      printf("Debug             ---------- d\n");
      printf("Provision DA obk  ---------- p\n");
      printf("Regression        ---------- r\n");
      printf("Show obk (HDPL1)  ---------- s\n");
      printf("Increment HDPL    ---------- h\n");
      printf("Increment OBKHDPL ---------- o\n");
      printf("Exit S systick on ---------- x\n");
      printf("Exit S systick off---------- z\n");
      ...
      case 'x':
        loop = 0;
        /* Resume systick to allow GPIO toggle from S side */
        HAL_ResumeTick();
        break;
      case 'z':
        loop = 0;
        /* S systick is still suspended */        
        break;
      default :
        printf("===> Bad input!\r\n");
        break;
