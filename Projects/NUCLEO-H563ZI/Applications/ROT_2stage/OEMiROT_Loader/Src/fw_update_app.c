...
static HAL_StatusTypeDef FW_UPDATE_APP_IMAGE(int is_oemurot);
#if !defined(MCUBOOT_OVERWRITE_ONLY)
static void FW_Install_AppImage(int is_oemurot);
#endif /* defined(MCUBOOT_OVERWRITE_ONLY) */
#if (DATA_IMAGE_PARTITION_SIZE > 0)
static HAL_StatusTypeDef FW_UPDATE_DATA_IMAGE(void);
#if !defined(MCUBOOT_OVERWRITE_ONLY)
static void FW_Install_DataImage(void);
#endif /* !defined(MCUBOOT_OVERWRITE_ONLY) */
  ...
  while (exit == 0U)
  {
    ...
    if (COM_Receive(&key, 1U, RX_TIMEOUT) == HAL_OK)
    {
      ...
      case '1' :
          ...
          break;
      case '2' :
          FW_UPDATE_APP_IMAGE(1); // oemurot
          break;
      case '3' :
          FW_UPDATE_APP_IMAGE(0); // app
          break;          
#if (DATA_IMAGE_PARTITION_SIZE > 0)
      case '4' :
          ...
          break;
#endif /* (DATA_IMAGE_PARTITION_SIZE > 0) */
#if (MCUBOOT_OVERWRITE_ONLY == 0) //&& (MCUBOOT_APP_IMAGE_NUMBER == 1)
      case 'a':
          FW_Install_AppImage(1); // oemurot
          break;
      case 'b':
          FW_Install_AppImage(0); // app
          break;          
#endif /* (MCUBOOT_OVERWRITE_ONLY) && defined(MCUBOOT_APP_IMAGE_NUMBER == 1) */
#if (MCUBOOT_OVERWRITE_ONLY == 0) && (DATA_IMAGE_PARTITION_SIZE > 0)
      case 'c':
          FW_Install_DataImage();
          break;
#endif /* (MCUBOOT_OVERWRITE_ONLY == 0) && (DATA_IMAGE_PARTITION_SIZE > 0) */
      case 'x' :
          exit = 1U;
          break;
      default:
...
static HAL_StatusTypeDef FW_UPDATE_APP_IMAGE(int is_oemurot)
{
  ...
  if ( is_oemurot == 1 )
  {
  #if (MCUBOOT_PRIMARY_ONLY == 1)
    /* Incase PRIMARY_ONLY is defined, we download to active slot directly */
    fw_image_dwl_area.DownloadAddr =  OEMuROT_IMAGE_PRIMARY_PARTITION_OFFSET;
    fw_image_dwl_area.MaxSizeInBytes = OEMuROT_IMAGE_PARTITION_SIZE;
  #else /* (MCUBOOT_PRIMARY_ONLY == 1) */
    fw_image_dwl_area.DownloadAddr =  OEMuROT_IMAGE_SECONDARY_PARTITION_OFFSET;
    fw_image_dwl_area.MaxSizeInBytes = OEMuROT_IMAGE_PARTITION_SIZE;
  #endif /* (MCUBOOT_PRIMARY_ONLY == 1) */    
  }
  else
  {
  #if (MCUBOOT_PRIMARY_ONLY == 1)
    /* Incase PRIMARY_ONLY is defined, we download to active slot directly */
    fw_image_dwl_area.DownloadAddr =  APP_IMAGE_PRIMARY_PARTITION_OFFSET;
    fw_image_dwl_area.MaxSizeInBytes = APP_IMAGE_PARTITION_SIZE;
  #else /* (MCUBOOT_PRIMARY_ONLY == 1) */
    fw_image_dwl_area.DownloadAddr =  APP_IMAGE_SECONDARY_PARTITION_OFFSET;
    fw_image_dwl_area.MaxSizeInBytes = APP_IMAGE_PARTITION_SIZE;
  #endif /* (MCUBOOT_PRIMARY_ONLY == 1) */
  }
...
static void FW_Install_AppImage(int is_oemurot)
{
  ...
  if ( is_oemurot == 1)
  {
    MagicAddress = APP_IMAGE_SECONDARY_PARTITION_OFFSET  + APP_IMAGE_PARTITION_SIZE - sizeof(MagicTrailerValue);
  }
  else
  {
    MagicAddress = APP_IMAGE_SECONDARY_PARTITION_OFFSET  + APP_IMAGE_PARTITION_SIZE - sizeof(MagicTrailerValue);
...
static HAL_StatusTypeDef FW_UPDATE_DATA_IMAGE(void)
{
  ...
  /* Get Info about the download area */
#if (MCUBOOT_PRIMARY_ONLY == 1)
  fw_image_dwl_area.DownloadAddr =  DATA_IMAGE_PRIMARY_PARTITION_OFFSET;
  fw_image_dwl_area.MaxSizeInBytes = DATA_IMAGE_PARTITION_SIZE;
#else  /* (MCUBOOT_PRIMARY_ONLY == 1) */
