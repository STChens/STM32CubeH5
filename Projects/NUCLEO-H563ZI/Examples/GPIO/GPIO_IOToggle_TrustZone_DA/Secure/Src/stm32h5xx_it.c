...
/* USER CODE END EV */
#define NS_RAM_START_ADDR 0x20050000

static void jump_to_ns_loop_in_ram(void)
{
#if 1
#define WHILE_1_OPCODE 0xe7fe
    typedef void (*nsfptr_t)(void) __attribute__((cmse_nonsecure_call));
    nsfptr_t nsfptr = (nsfptr_t)(NS_RAM_START_ADDR + 1);
    __IO uint16_t *pt = (uint16_t *)NS_RAM_START_ADDR;
