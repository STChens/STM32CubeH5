...
::=============================================================================================
set s_code_xml="%provisioningdir%\%bootpath%\Images\%project%_S_Code_Image_presign.xml"
set s_code_init_xml="%provisioningdir%\%bootpath%\Images\%project%_S_Code_Init_Image_presign.xml"
...
::=================================================================================================
set bin_path_xml_field="..\..\..\Applications\ROT_2stage\OEMuROT_Appli\Binary"
...
set s_app_enc_sign_digest_xml_field="%bin_path_xml_field%\rot_tz_s_app_enc_sign.digest"
set s_app_init_sign_digest_xml_field="%bin_path_xml_field%\rot_tz_s_app_init_sign.digest"
...
:postbuild
echo Postbuild %signing% image (step1) >> %current_log_file% 2>>&1
...
::update xml file : output file (output digest for signing by external too)
%python%%applicfg% xmlval -v %s_app_enc_sign_digest_xml_field% --string -n %fw_digst_out_bin_xml_field% %s_code_xml% --vb >> %current_log_file% 2>>&1
if !errorlevel! neq 0 goto :error
...
%python%%applicfg% xmlval -v %s_app_init_sign_digest_xml_field% --string -n %fw_digst_out_bin_xml_field% %s_code_init_xml% --vb >> %current_log_file% 2>>&1
if !errorlevel! neq 0 goto :error
...
%stm32tpccli% -pb %s_data_init_xml% >> %current_log_file% 2>>&1
if !errorlevel! neq 0 goto :error
:no_s_data
