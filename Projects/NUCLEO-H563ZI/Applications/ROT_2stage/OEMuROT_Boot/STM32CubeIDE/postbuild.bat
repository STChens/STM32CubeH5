...
set s_code_presign_xml="%provisioningdir%\%bootpath%\Images\%project%_S_Code_Image_presign.xml"
...
set s_code_init_presign_xml="%provisioningdir%\%bootpath%\Images\%project%_S_Code_Init_Image_presign.xml"
...
echo app_full_secure=%app_full_secure% >> %current_log_file% 2>&1

IF "%app_full_secure%" == "1" (
set s_ld_file=%appli_dir%\STM32CubeIDE\STM32H563ZITX_FLASH.ld
...
:: BOOTADDR OB shall be set for OEMiROT, OEMuROT does not change the boot address
...
:: WRP and HDP OBs shall be set for OEMiROT only, OEMuROT does not touch those settings
...
:: do the same for %s_code_init_presign_xml% except for --confirm
...
:: do the same for %s_code_presign_xml%
...
:: ================================================================ Update %s_main% ===================================================================
set "command=%python%%applicfg% definevalue --layout %preprocess_bl2_file% -m RE_AREA_0_OFFSET -n APP_CODE_OFFSET %s_main% --vb >> %current_log_file% 2>&1"
%command%
IF !errorlevel! NEQ 0 goto :error

set "command=%python%%applicfg% definevalue --layout %preprocess_bl2_file% -m RE_IMAGE_FLASH_SECURE_IMAGE_SIZE -n APP_CODE_SIZE %s_main% --vb >> %current_log_file% 2>&1"
%command%
IF !errorlevel! NEQ 0 goto :error
